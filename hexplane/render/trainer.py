...
from hexplane.model.s3im import S3IM  #GGGGGGGGGGGGGGGGGG
...
class Trainer:
    ...
    def sample_data(self, train_dataset, iteration):
        ...
        if self.cfg.data.datasampler_type == "rays":
            ...
            rays_train, rgb_train, frame_time, frame_idx = (
                data["rays"],
        ...
        # sample images: randomly pick one image from the training dataset and sample a batch of rays from all the rays of the image.
            ...
            frame_idx = frame_idx[select_inds]
            if self.depth_data:
        ...
        # hierarchical sampling from dyNeRF: hierachical sampling involves three stages of samplings.
            ...
            if iteration <= self.cfg.data.stage_1_iteration:
                ...
                rgb_train = rgb_train[select_inds]
                frame_time = torch.ones_like(rays_train[:, 0:1]) * frame_time
        return rays_train, rgb_train, frame_time, frame_idx, train_depth
    ...
    def train(self):
        ...
        if (self.cfg.data.datasampler_type == "rays") and (ndc_ray is False):
            ...
            allrays, allrgbs, alltimes, allidxs, alldepths = model.filtering_rays(
                allrays, allrgbs, alltimes, allidxs, alldepths, bbox_only=True
            )
        ...
        mean_rgb = allrgbs.mean(dim=0).to(self.device)
        for iteration in pbar:
            # Sample dat
            rays_train, rgb_train, frame_time, frame_idx, depth = self.sample_data(
            ...
            # s3im loss #GGGGGGGGGGGGGGGGGGGGGGGGGGGGG
            s3im_func = S3IM(
                kernel_size=self.cfg.model.s3im_kernel, stride=self.cfg.model.s3im_stride, repeat_time=self.cfg.model.s3im_repeat_time,
                patch_height=self.cfg.model.s3im_patch_height, patch_width=self.cfg.model.s3im_patch_width).cuda()

            if(self.cfg.model.s3im_weight > 0):
                loss_s3im = s3im_func(rgb_map, rgb_train) * self.cfg.model.s3im_weight
                total_loss = total_loss+loss_s3im
            ...
            if self.cfg.model.L1_constraint > 0:
                # loss_constraint = rgbh_map.mean()
                # larger = rgbh_map[rgbh_map > 7].mean()
                # miner = rgbh_map[rgbh_map< -7].mean()
                total_loss += self.cfg.model.L1_constraint * model.tonemapper.zero_point_contraint(mean_rgb,torch.zeros([1,1]).to(self.device))
                # total_loss += self.cfg.model.L1_constraint * (torch.abs(larger)+torch.abs(miner))
