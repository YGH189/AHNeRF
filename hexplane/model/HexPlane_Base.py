...
from hexplane.model.tonemapper import ToneMapper
...
class PoseEmbedding(torch.nn.Embedding):
    def __init__(self):
        super(PoseEmbedding, self).__init__(1, 9)
        self.weight = torch.nn.Parameter(torch.eye(3, 3).reshape(1,9))
...
class HexPlane_Base(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tonemapper = ToneMapper(64).to(device)
    ...
    def sample_rays(
            self,
            rays_o: torch.Tensor,  # 光线的起点，形状为 (B, 3)
            rays_d: torch.Tensor,  # 光线的方向，形状为 (B, 3)
            is_train: bool = True,  # 是否处于训练模式
            N_samples: int = -1,  # 每条光线的采样点数，如果为 -1 则使用默认值 self.nSamples
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Sample points along rays based on the given ray origin and direction.
        ...
        # 设置采样点数
        N_samples = N_samples if N_samples > 0 else self.nSamples
        near, far = self.near_far  # 获取近远平面，定义光线的采样范围

        # 初始均匀采样
        coarse_interpx = torch.linspace(near, far, N_samples).unsqueeze(0).to(rays_o)

        if is_train:
            # 在训练模式下，为每个初始采样点添加均匀扰动
            coarse_interpx += torch.rand_like(coarse_interpx).to(rays_o) * ((far - near) / N_samples)

        # 计算每条光线上的初始采样点，形状为 (B, N_samples, 3)
        coarse_rays_pts = rays_o[..., None, :] + rays_d[..., None, :] * coarse_interpx[..., None]

        # 细化采样：基于粗采样点生成高斯分布的采样点
        if is_train:
            fine_interpx = coarse_interpx + torch.randn_like(coarse_interpx).to(rays_o) * ((far - near) / N_samples / 4)
        else:
            fine_interpx = coarse_interpx

        # 计算每条光线上的细化采样点，形状为 (B, N_samples, 3)
        fine_rays_pts = rays_o[..., None, :] + rays_d[..., None, :] * fine_interpx[..., None]

        # 判断细化采样点是否在轴对齐包围盒（AABB）之外
        mask_outbbox = ((self.aabb[0] > fine_rays_pts) | (fine_rays_pts > self.aabb[1])).any(dim=-1)

        # 返回细化采样点、细化采样点距离光线起点的距离和包围盒内点的掩码
        return fine_rays_pts,fine_interpx, ~mask_outbbox
    ...
    def get_mutiexposures_index(self):
        render_exposures = self.get_render_exposures()
        index = torch.arange(0, self.train_images).to(self.device)
        exp_min = render_exposures[0]
        idx_min = 0
        exp_max = render_exposures[0]
        idx_max = 0
        for idx, exp in zip(index, render_exposures):
            if exp < exp_min:
                exp_min = exp
                idx_min = idx
            if exp > exp_max:
                exp_max = exp
                idx_max = idx
        return (idx_min, exp_min), (idx_max, exp_max)
    def get_render_exposures(self):
        query = torch.arange(0, self.train_images).to(self.device)
        exp1 = self.exposure_embedding(query)
        # exp1 = self.exposure_linear(exp1)
        return exp1
    def get_hdr_render_exposures(self):
        return self.get_render_exposures().mean()
    def compute_exposure(self,frameidx):
        # return self.exposure_embedding(frameidx)
        exp_embedding = torch.tensor(torch.ones_like(frameidx),dtype=torch.float32)

        exp_embedding_compat = torch.where(frameidx!=0,True,False)
        exp_embedding[exp_embedding_compat] = self.exposure_embedding(frameidx[exp_embedding_compat]).squeeze()
        # exp_embedding = torch.zeros_like(exp_embedding)
    ...
    def forward(
        ...
        """
        # Prepare rays.
        # othervideo_mask = torch.where(self.video_split_index[frame_idx] == 0,False,True).squeeze()
        viewdirs = rays_chunk[:, 3:6]
        # rays_chunk[:, :3][othervideo_mask] = rays_chunk[:, :3][othervideo_mask]@(self.pose_affine(self.query).reshape(3,3))
        # viewdirs[othervideo_mask] = viewdirs[othervideo_mask]@(self.pose_affine(self.query).reshape(3,3))
        ...
        # add time shift mask
        # frame_time[othervideo_mask] += self.time_affine(self.time_embedding(frame_idx[othervideo_mask])).squeeze()
        # frame_time[othervideo_mask] += self.time_affine(self.query).squeeze()
        ...
        # rgb_h = self.tonemapper(rgb, render_exp.to(self.device), None)
        # rgb_h = rgb_l
        acc_map = torch.sum(weight, -1)
        rgb_h = torch.exp(rgb)
        rgb_map = torch.sum(weight[..., None] * rgb_l, -2)
        rgb_h_map = torch.sum(weight[..., None] * rgb_h, -2)
        
        # If white_bg or (is_train and torch.rand((1,))<0.5):
        if white_bg or not is_train:
            rgb_map = rgb_map + (1.0 - acc_map[..., None])
            rgb_h_map = rgb_h_map + (1.0 - acc_map[..., None])
        else:
            rgb_map = rgb_map + (1.0 - acc_map[..., None]) * torch.rand(
                size=(1, 3), device=rgb_map.device
            )
            rgb_h_map = rgb_h_map + (1.0 - acc_map[..., None]) * torch.rand(
                size=(1, 3), device=rgb_h_map.device
            )
