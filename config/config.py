...
class Model_Config:
    ...
    '''s3im {0.05, 0.1, 0.2, 0.5, 1, 2, 5}'''
    s3im_weight: float = 0.05
    s3im_kernel: int = 4
    s3im_stride: int = 4
    s3im_repeat_time: int = 10
    s3im_patch_height: int = 64
    s3im_patch_width: int = 64
...
class Data_Config:
    ...
    datadir1: str = "./video1"
    datadir2: str = "./video2"
    dataset_name: str = "dnerf"  # choose from "dnerf", "neural3D_NDC", "blender"
    downsample: float = 1.0
    cal_fine_bbox: bool = True
    N_vis: int = -1
...
class Optim_Config:
    ...
    lr_exposure_embedding: float = 0.02
    lr_exposure_linear: float = 0.001
    lr_time_affine: float = 0.02
    lr_pose_affine: float = 0.02
    lr_time_embedding: float = 0.02
    lr_tonemapper : float = 0.02
